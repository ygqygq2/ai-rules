...
### [106. Lua 游戏开发最佳实践](./106-lua-game.mdc)

**适用场景：**

- Lua 游戏开发（Unity/Cocos/Love2D 等）
- 需要管理大量配置数据、实体 ID、关联关系的游戏
- 多系统协作的中大型游戏架构

**核心内容：**

- 数据表管理架构（配置表标准结构、ConfigManager 单例）
- 运行时实体管理（ID 系统、EntityRegistry、实体注册）
- 数据与配置关联（ID 引用、弱引用关联、跨表关联）
- 事件驱动系统解耦（EventBus、系统间 ID 通信）
- 模块化架构（目录结构、初始化流程）
- 性能优化（配置表缓存、二级索引、对象池）
- ID 驱动设计（配置 ID、实体 ID、事件传递 ID）
